...
describe('application reducer', () => {
  ...
  describe('updateBlockNumber', () => {
    it('updates block number', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.HARMONY, blockNumber: 4 }))
      expect(store.getState().blockNumber[ChainId.HARMONY]).toEqual(4)
    })
    it('no op if late', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.HARMONY, blockNumber: 2 }))
      expect(store.getState().blockNumber[ChainId.HARMONY]).toEqual(3)
    })
