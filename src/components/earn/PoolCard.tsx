...
import { ETHER, JSBI/** , TokenAmount*/ } from 'harmony-swap-sdk'
  ...
  return (
    ...
    <Wrapper showBackground={isStaking} bgColor={backgroundColor}>
      ...
      <TopSection>
        ...
        <StyledInternalLink to={`/ONE/${currencyId(currency0)}/${currencyId(currency1)}/${stakingInfo.index}`} style={{ width: '100%' }}>
      ...
      <StatContainer>
        ...
        <RowBetween>
          ...
          <TYPE.white>{`${stakingInfo.totalRewardRate
            ?.multiply(`${60 * 60 * 24}`)
            ?.toFixed(0, { groupSeparator: ',' })} ONE / day`}</TYPE.white>
        </RowBetween>
        <RowBetween>
          ...
          <BottomSection showBackground={true}>
            ...
            <TYPE.black style={{ textAlign: 'right' }} color={'white'} fontWeight={500}>
              ...
              {`${stakingInfo.rewardRate
                ?.multiply(`${60 * 60 * 24}`)
                ?.toSignificant(4, { groupSeparator: ',' })} ONE / day`}
            </TYPE.black>
          </BottomSection>
        </>
      )}
